...
RC InsertStmt::create(Db *db, const Inserts &inserts, Stmt *&stmt)
{
  ...
  for (int i = 0; i < value_num; i++) {
    ...
    // const char* temp_char_value = (char*)values[i].data;
    // std::cout << (char*)values[i].data << std::endl;
    if (field_type != value_type) { // TODO try to convert the value type to field type
      // LOG_WARN("field type mismatch. table=%s, field=%s, field type=%d, value_type=%d", 
      //          table_name, field_meta->name(), field_type, value_type);
      // return RC::SCHEMA_FIELD_TYPE_MISMATCH;
      if (field_type == INTS && value_type == FLOATS) {
        *(int*)(values[i].data) = (*(float*)(values[i].data) + 5) / 10;
        // const void* p = &values[i].type;
        // memcpy(p, &field_type, sizeof(field_type));
        // values[i].type = field_type;
        // *(values[i].data) = (*(values[i].data) * 10 + 5) / 10;
      } else if (field_type == FLOATS && value_type == INTS) {
        *(float*)(values[i].data) = *(int*)values[i].data;
      } else if (field_type == CHARS && value_type == INTS) {
        // char temp_chars[15];
        // int temp_int = *((int*)(values[i].data));
        // int flag = (temp_int < 0);
        // int ind = flag;
        // if (temp_int == 0) {
        //   temp_chars[ind] = '0';
        ...
        //   }
        //   while (temp_int) {
        //     temp_chars[ind] = temp_int % 10 + '0';
        //     temp_int /= 10;
        //     ind++;
        //   }
        //   //123 1234 -123 -1234
        //   // 逆序
        //   for (int i = 0; i < (ind - flag) / 2; i++) {
        //     if (flag) {
        //       char t = temp_chars[i+1];
        //       temp_chars[i+1] = temp_chars[ind-i-1];
        //       temp_chars[ind-i-1] = t;
        //     } else {
        //       char t = temp_chars[i];
        //       temp_chars[i] = temp_chars[ind-1-i];
        //       temp_chars[ind-1-i] = t;
        //     }
        //   }
        // }
        // temp_chars[ind] = '\0';
        // memcpy(values[i].data, temp_chars, ind);
